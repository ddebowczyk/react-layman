...
selections, invalid geometry, non-finite numbers, cyclic data, and non-JSON
tab data are rejected. Layman does not silently migrate a snapshot. The host
must perform any earlier-format migration before it calls `deserializeState`.
